import { formatSageError, hasSageBridge } from '@sage-app/sdk';
import { PageShell } from '../components/PageShell';
import { addLog } from '../lib/logStore';
import { useSageClient } from '../hooks/useSageClient';

// Vulnerability location in the host app:
//   sage/crates/sage-apps/src/bridge/mod.rs  lines 427–442
//
// The bridge stores each approval request in an unbounded BTreeMap keyed by UUID
// and emits `apps:bridge-approval-requested` immediately, returning Pending{} with
// no rate-limiting, deduplication, or per-app cap on simultaneous pending approvals.
//
// Dialogs are presented serially (one at a time), so the attack is approval fatigue
// (CWE-799) rather than a UI overlay: the attacker fires N requests (O(1) cost) and
// waits for the user to slip once across N sequential dismissals (O(N) cost).
// Granting any single dialog permanently grants the capability; the rest resolve alreadyGranted.
...
{`let already_pending = pending
    ...
    .any(|p| p.app.id() == app_model.id()
          ...
          && p.request.params_json == request.params_json);

if already_pending {
    return Ok(RustBridgeInvokeResult::Immediate {
        response: failure(
            expected_channel, &request.id,
            "approval_pending",
